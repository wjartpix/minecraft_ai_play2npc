...
| `tts.voice` | CosyVoice 音色，默认 `longanhuan`（龙安欢，欢脱元气女），详见 [1.7 节音色列表](#17-配置-tts-语音合成可选) |
| `tts.model` | TTS 模型名称，必须与 `voice` 版本匹配：`cosyvoice-v3-flash`/`cosyvoice-v2`/`cosyvoice-v1` |
...
> **默认配置模板：** `src/main/resources/playerengine-llm-default.json`，如需修改默认值可编辑此文件后重新构建。
...
**默认配置：** `cosyvoice-v3-flash` 模型 + `longanhuan` 音色（龙安欢，欢脱元气女）。
...
| 模型 | 可用音色格式 | 示例 |
|------|-------------|------|
| `cosyvoice-v3-flash` | `long*`、`long*_v3`、`loong*_v3` | `longanhuan`、`longanyang`、`longhuhu_v3` |
| `cosyvoice-v2` | `*_v2` | `longxiaochun_v2`、`longyuan_v2` |
| `cosyvoice-v1` | 无后缀 | `longwan` |

> **重要：** 模型与音色必须版本匹配！混用会导致 `418` 错误（InvalidParameter）。修改 `voice` 时务必同步修改 `model`。

---

#### cosyvoice-v3-flash 完整音色列表

| 音色 ID | 名称 | 特质 | 年龄 | 语言 | 场景 |
|---------|------|------|------|------|------|
| `longanyang` | 龙安洋 | 阳光大男孩 | 20~30岁 | 中/英 | 社交陪伴（标杆音色） |
| `longanhuan` | 龙安欢 | 欢脱元气女 | 20~30岁 | 中/英 | 社交陪伴（标杆音色，默认） |
| `longhuhu_v3` | 龙呼呼 | 天真烂漫女童 | 6~10岁 | 中/英 | 童声（标杆音色） |
| `longpaopao_v3` | 龙泡泡 | 飞天泡泡音 | 6~15岁 | 中/英 | 智能玩具/儿童故事机 |
| `longjielidou_v3` | 龙杰力豆 | 阳光顽皮男 | 10岁 | 中/英 | 智能玩具/儿童故事机 |
| `longxian_v3` | 龙仙 | 豪放可爱女 | 12岁 | 中/英 | 智能玩具/儿童故事机 |
| `longling_v3` | 龙铃 | 稚气呆板女 | 10岁 | 中/英 | 智能玩具/儿童故事机 |
| `longshanshan_v3` | 龙闪闪 | 戏剧化童声 | 6~15岁 | 中/英 | 消费电子-儿童有声书 |
| `longniuniu_v3` | 龙牛牛 | 阳光男童声 | 6~15岁 | 中/英 | 消费电子-儿童有声书 |
| `longjiaxin_v3` | 龙嘉欣 | 优雅粤语女 | 30~35岁 | 粤语/英 | 方言 |
| `longjiayi_v3` | 龙嘉怡 | 知性粤语女 | 25~30岁 | 粤语/英 | 方言 |
| `longanyue_v3` | 龙安粤 | 欢脱粤语男 | 25~35岁 | 粤语/英 | 方言 |
| `longlaotie_v3` | 龙老铁 | 东北直率男 | 25~30岁 | 东北话/英 | 方言 |
| `longshange_v3` | 龙陕哥 | 原味陕北男 | 25~35岁 | 陕西话/英 | 方言 |
| `longanmin_v3` | 龙安闽 | 清纯萝莉女 | 18~25岁 | 闽南话/英 | 方言 |
| `loongkyong_v3` | — | 韩语女 | 25~30岁 | 韩语 | 出海营销（仅限北京地域） |
| `loongriko_v3` | — | 二次元霓虹女 | 18~25岁 | 日语 | 出海营销（仅限北京地域） |
| `loongtomoka_v3` | — | 日语女 | 30~35岁 | 日语 | 出海营销（仅限北京地域） |

> v3-flash 音色支持 **SSML**、**Instruct**（情感/场景/角色控制）和**时间戳**功能。详情参见 [阿里云 CosyVoice 音色列表](https://help.aliyun.com/zh/model-studio/cosyvoice-voice-list)。

---

#### cosyvoice-v2 音色列表（备用）

如想使用 v2 模型，需将 `model` 改为 `cosyvoice-v2`，并选用以下 `*_v2` 音色：

| 音色 ID | 名称 | 特点 |
|---------|------|------|
| `longxiaochun_v2` | 龙小淳 v2 | 女声，温柔亲切 |
...
**注意事项：**
- 录音至少需要 **0.5 秒**，否则提示"录音时间太短"
- 录音最长 60 秒，超时自动停止
- 支持 **VAD 自动断句**：说完话后约 0.6 秒自动结束录音并发送，无需刻意长按 V 键
...
3. 对着麦克风说话（至少 0.5 秒）
4. **松开 V 键** 结束录音，或说完话后等待约 0.6 秒自动结束（VAD 自动断句），语音自动发送识别
...
| V 键按下去就停 | PTT 需要按住不放，至少 0.5 秒后松开；或启用 VAD 自动断句 |
