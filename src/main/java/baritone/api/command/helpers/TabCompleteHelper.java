...
public class TabCompleteHelper {
   ...
   public TabCompleteHelper addCommands() {
      return this.append(ICommandManager.registry.descendingStream().flatMap(command -> command.getNames().stream()).distinct());
   }

   public TabCompleteHelper addSettings(Settings settings) {
      return this.append(
         settings.allSettings.stream().map(Settings.Setting::getName).filter(s -> !s.equalsIgnoreCase("logger")).sorted(String.CASE_INSENSITIVE_ORDER)
      );
   }

   public TabCompleteHelper addModifiedSettings(Settings settings) {
      return this.append(SettingsUtil.modifiedSettings(settings).stream().map(Settings.Setting::getName).sorted(String.CASE_INSENSITIVE_ORDER));
   }

   public TabCompleteHelper addToggleableSettings(Settings settings) {
      return this.append(settings.getAllValuesByType(Boolean.class).stream().map(Settings.Setting::getName).sorted(String.CASE_INSENSITIVE_ORDER));
   }
}
